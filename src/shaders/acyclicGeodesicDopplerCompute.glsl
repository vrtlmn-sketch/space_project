...
// Refine the step near free-object meshes: take small steps through the
...

        {
            ...
            if (segLen > 0.0001)
            {
                ...
                if (segHit >= 0)
                {
                    ...
                    for (int i = 0; i < uObjectCount; i++)
                    {
                        ...
                        if (otype == 1)
                        {
                            ...
                            float core = (d2 < srad2) ? 5.0 : 5.0 * exp(-(d2 - srad2) / (srad2 * 0.35));
                ...
                for (int i = 0; i < uObjectCount; i++)
                {
                    ...
                    {
                        float srad = objects[i].radius, srad2 = srad * srad;
                        float core = (sd2 < srad2) ? 5.0 : 5.0 * exp(-(sd2 - srad2) / (srad2 * 0.35));
                        float coronaR = srad * 3.5;
    ...
    if (uDustStrength > 0.0) {
        ...
        for (int di = 0; di < uObjectCount; di++) {
            ...
            { float dInfl2 = uDustInfluence * uDustInfluence; if (dd2 < dInfl2 * 9.0 && hash1(floor((dcen - uDustCenter) / max(uDustInfluence * 3.0, 1e-6))) < uDustCoverage) dustTau += objects[di].mass * exp(-dd2 / dInfl2); }
        }
