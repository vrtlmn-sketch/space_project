...
void main()
{
    ...
    // Per-particle dust (converged with Simple RT): puff radius + column. Each
    // particle contributes ONCE — in the segment that owns its closest approach —
    ...
    for (int step = 0; step < uMaxSteps; step++)
    {
        ...
        if (radialVel > 0.0 && accelMag * max(nearRs, 1e-9) < BH_ESCAPE_ACCEL_RS)
        {
            break;
        }
...
// Refine the step near free-object meshes: take small steps through the
        ...
        if (segLen > 0.0001)
        {
            ...
            if (segHit >= 0)
            {
                ...
                for (int i = 0; i < uObjectCount; i++)
                {
                    ...
                    {
                        // Converged star glow + per-particle dust, counted ONLY in
                        // the segment that owns this particle's closest approach.
                        float tAlong = dot(cen - prevPos, subDir);
                        bool  owns   = (tAlong >= 0.0 && tAlong < subLen)
                                    || (step == 0 && tAlong < 0.0);
                        if (owns) {
                            float bd2 = (tAlong < 0.0) ? dot(cen - prevPos, cen - prevPos) : d2;
                            float hot;
                            vec3  gcol = gxStarColor(float(i), objects[i].temperature, hot);
                            float coreAmt;
                            float hazeAmt = pointSourceGlow(bd2, cen, objects[i].radius, float(i), coreAmt);
                            curvedGlow += gcol * (hazeAmt + coreAmt);
                            curvedGlow += gxGasGlow(float(i), bd2, hot);
        ...
        for (int i = 0; i < uObjectCount; i++)
        {
            ...
            {
                // Converged star glow + per-particle dust — segment-ownership
                // gated (see hit branch). sd2 is the true closest-approach b²
                // whenever the parameter lies inside this segment.
                ...
                if (owns) {
                    ...
                    curvedGlow += gcol * (hazeAmt + coreAmt);
                    curvedGlow += gxGasGlow(float(i), sd2, hot);
                    float pdust = objects[i].color.x;
    ...
    if (!hitScene)
    {
        ...
        for (int i = 0; i < uObjectCount; i++)
        {
            ...
            {
                // Converged star glow + per-particle dust on the escaped straight
                // ray. Particles already owned by marched segments sit BEHIND the
                // escape origin, so their clamped d2 is huge → they self-suppress.
                ...
                if (tE >= 0.0) {
                    ...
                    curvedGlow += gcol * (hazeAmt + coreAmt);
                    curvedGlow += gxGasGlow(float(i), d2, hot);
                    float pdust = objects[i].color.x;
    ...
    if (hitScene && hitIdx >= 0)
    {
        ...
        else if (otype == 5)
        {
            ...
            color = shadePlanet(ro, hitPos, hitNorm, N, base, vec3(0.0),
                                hitNorm, objects[hitIdx].color.xyz, objects[hitIdx].rotation, vec4(0.0), vec4(0.0));
        }
        else
        {
