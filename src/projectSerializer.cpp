                             ...
                             const std::vector<UniverseRecord>& universes)
{
  ...
  // ── Scene settings ──
  {
    ...
    s["starFieldStops"]    = settings.starFieldStops;
    s["pointObjectStops"]  = settings.pointObjectStops;
    s["autoExposure"]      = settings.autoExposure;
    s["aeLimit"]           = settings.aeLimit;
    s["aeMaxDarkenStops"]  = settings.aeMaxDarkenStops;
    s["aeHiLimit"]         = settings.aeHiLimit;
    s["aeHiSize"]          = settings.aeHiSize;
    s["aeHiPower"]         = settings.aeHiPower;
    s["aeSpeedStops"]      = settings.aeSpeedStops;
...
ProjectData ProjectSerializer::Load(const std::string& path)
{
  ...
  if (root.contains("settings")) {
    ...
    st.bloomThreshold     = s.value("bloomThreshold",     SceneSettings{}.bloomThreshold);
    ...
    st.spikeCount         = s.value("spikeCount",         SceneSettings{}.spikeCount);
    st.spikeAngle         = s.value("spikeAngle",         SceneSettings{}.spikeAngle);
    st.spikeLength        = s.value("spikeLength",        SceneSettings{}.spikeLength);
    st.spikeDecay         = s.value("spikeDecay",         SceneSettings{}.spikeDecay);
    st.spikeSecondary     = s.value("spikeSecondary",     SceneSettings{}.spikeSecondary);
    st.spikeChroma        = s.value("spikeChroma",        SceneSettings{}.spikeChroma);
    ...
    st.starFieldStops     = s.value("starFieldStops",     SceneSettings{}.starFieldStops);
    st.pointObjectStops   = s.value("pointObjectStops",   SceneSettings{}.pointObjectStops);
    st.autoExposure       = s.value("autoExposure",       SceneSettings{}.autoExposure);
    st.aeLimit            = s.value("aeLimit",            SceneSettings{}.aeLimit);
    st.aeMaxDarkenStops   = s.value("aeMaxDarkenStops",   SceneSettings{}.aeMaxDarkenStops);
    st.aeHiLimit          = s.value("aeHiLimit",          SceneSettings{}.aeHiLimit);
    st.aeHiSize           = s.value("aeHiSize",           SceneSettings{}.aeHiSize);
    st.aeHiPower          = s.value("aeHiPower",          SceneSettings{}.aeHiPower);
    st.aeSpeedStops       = s.value("aeSpeedStops",       SceneSettings{}.aeSpeedStops);
